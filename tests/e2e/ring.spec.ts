...
test('decorated ring styles update the canvas and survive reload', async ({ page }) => {
  await page.goto('')
  await addRing(page)
  const ring = page.locator('[data-testid^="ring-layer-"]').first()
  await page.getByLabel('Ring style', { exact: true }).selectOption('concentric')
  await expect(ring.locator('circle')).toHaveCount(6)
  await page.getByLabel('Ring count', { exact: true }).fill('5')
  await page.getByLabel('Ring count', { exact: true }).press('Tab')
  await expect(ring.locator('circle')).toHaveCount(10)
  await page.getByLabel('Ring style', { exact: true }).selectOption('divided')
  await expect(ring.locator('circle')).toHaveCount(4)
  await expect(ring.locator('path')).toHaveCount(2)
  await page.getByLabel('Ring style', { exact: true }).selectOption('arc')
  await expect(ring.locator('circle')).toHaveCount(0)
  await expect(ring.locator('path')).toHaveCount(2)
  await expect
    .poll(() =>
      page.evaluate(() =>
        Object.values(localStorage).some((value) => value.includes('"style":"arc"'))
      )
    )
    .toBe(true)
  await page.reload()
  await expect(page.locator('[data-testid^="ring-layer-"]').first().locator('path')).toHaveCount(2)
})
